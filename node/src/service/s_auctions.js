...
export const AuctionsService = {
  ...
  async getAll() {
    ...
    const res = await db.query(`SELECT a.auction_id, p.product_name, p.main_image_path, a.quantity, s.store_name, a.starting_price, a.current_price, a.start_time, a.end_time, COUNT(ab.bid_id) AS bidders_amount, a.status_auction
                                FROM auctions a
  ...
  async stop(id) {
    ...
    if (bidRes.rows.length > 0) {
      ...
      const shippingAddress = userRes.rows[0]?.address || 'Alamat belum diset';
      ...
      await db.query(
        `INSERT INTO order_items (order_id, product_id, quantity, price_at_order, subtotal)
         VALUES ($1, $2, $3, $4, $5)`,
        [orderId, auction.product_id, auction.quantity, finalPrice, finalPrice]
      );
    }
